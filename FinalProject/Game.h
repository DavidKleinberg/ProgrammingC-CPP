...
public:
    ...
    void alternate(Terminal::Color& c1, Terminal::Color& c2);

    void padTiles(Terminal::Color color1, Terminal::Color color2, Terminal::Color frame) const;

    void frameTopBottom(Terminal::Color color) const;
    void frameSides(Terminal::Color color) const;
    ...
    bool getBoardStatus() { return boardStatus; }

    void changeBoardStatus() {
        if (boardStatus) {
            boardStatus = false;
        } else {
            boardStatus = true;
...
protected:
    ...
    // Get the 2d position from the 1d index
    Position getPos(unsigned int index) const;
...
class Piece {
public:
  //Add checkDirection(Piece*, int direction)
  //if direction == UP_RIGHT { y=1, x=1};
  //if direction == UP { y=1,x=0};
  //if direction == UP_LEFT {y=1,x=-1};
